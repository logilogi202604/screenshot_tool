...
UDP 广播不是在每种网络下都能通——**跑着 TUN 模式的代理时默认路由会被它接管**，
广播可能被送到虚拟网卡上出不去。这种情况直接在配置里写死对方 IP：

```json
"fileshare_peers": ["192.168.1.50"]
```

写死的 peer **永远出现在发送菜单里**，不依赖广播。带端口写成 `"192.168.1.50:45318"`。
...
- **传文件的自动发现依赖 UDP 广播**，不是所有网络都通：路由器隔离客户端（AP isolation）、
  或跑着接管默认路由的 TUN 代理时，广播可能出不去。此时在
